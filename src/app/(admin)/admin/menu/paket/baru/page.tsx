...
/**
 * src/app/(admin)/admin/menu/paket/baru/page.tsx — /admin/menu/paket/baru
 * Form tambah paket baru beserta relasi menu
 */
...
export default function AdminPaketBaruPage() {
  ...
  useEffect(() => {
    fetch('/api/v1/admin/menu?per_page=100')
      ...
      .catch(() => showToast('Gagal memuat daftar menu.', 'error'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/admin/paket', {
        method: 'POST',
      ...
      if (res.ok && json.status === 'success') {
        ...
        showToast(json.message ?? 'Gagal membuat paket.', 'error')
      }
    } catch {
    ...
    <div className="min-h-screen bg-gray-50 pb-20 font-sans antialiased text-[#1E1E1E]">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-30 shadow-2xs">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <h1 className="text-base font-extrabold text-neutral-dark">Tambah Paket Baru</h1>
        </div>
      </header>
      ...
      {toast.show && (
        ...
        >
          <div className="shrink-0">
            {toast.type === 'success' ? (
              <i className="fa-solid fa-check text-[#4DAF48] text-xs" />
            ) : (
              <i className="fa-solid fa-circle-exclamation text-danger text-xs" />
            )}
          </div>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 mt-8">
        ...
        >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              ...
              {photos.length < 5 && (
                <label className="relative aspect-4/3 rounded-lg border-2 border-dashed border-gray-200 hover:border-[#4DAF48] bg-gray-50/50 hover:bg-[#F0FDF4]/30 flex flex-col items-center justify-center cursor-pointer transition-all gap-1.5 select-none shadow-2xs text-center p-2 group">
                  <i className="fa-solid fa-cloud-arrow-up text-lg text-gray-400 group-hover:text-[#4DAF48] transition-colors" />
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <div className="relative">
                <i className="fa-solid fa-magnifying-glass absolute left-3 top-[14px] text-neutral-mid text-sm" />
                <input
                  type="text"
                  value={searchMenu}
